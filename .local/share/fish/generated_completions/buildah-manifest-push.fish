...
complete -c buildah-manifest-push -l add-compression -d 'Makes sure that requested compression variant for each platform is added to t…'
...
complete -c buildah-manifest-push -l compression-format -d 'Specifies the compression format to use'
complete -c buildah-manifest-push -l compression-level -d 'Specify the compression level used with the compression'
...
complete -c buildah-manifest-push -l force-compression -d 'If set, push uses the specified compression algorithm even if the destination…'
