...
complete -c buildah-commit -l change -s c -d 'Apply the change to the committed image that would have been made if it had b…'
complete -c buildah-commit -l config -d 'Read a JSON-encoded version of an image configuration object from the specifi…'
complete -c buildah-commit -l creds -d 'The [username[:password]] to use to authenticate with the registry if required'
complete -c buildah-commit -l cw -d 'Produce an image suitable for use as a confidential workload running in a tru…'
